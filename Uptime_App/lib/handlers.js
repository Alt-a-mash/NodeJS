  ...
  if (data.method == 'get') {
    ...
    if (trimmedAssetName.length > 0) {
      ...
      helpers.getStaticAsset(trimmedAssetName, function(err, data) {
        if (!err && data) {
          // Detemine the content type (defailt to plain text)
      ...
      if (phone) {
        if (password) {
          if (tosAgreement) {
            ...
            callback(400, {'Error' : 'Missing tosAgreement field'});
          }
        } else {
          callback(400, {'Error' : 'Missing password field'});
        }
      } else {
        callback(400, {'Error' : 'Missing phone field'});
      }
    } else {
      callback(400, {'Error' : 'Missing lastName field'});
    }
  } else {
    callback(400, {'Error' : 'Missing firstName field'});
  }
};
    ...
    _data.read('users', phone, function(err, userData) {
      if (!err && userData) {
        ...
        if (hashedPassword == userData.hashedPassword) {
          // If valid, create a new token with a random name. Set expiration time 1 hour in the future
              ...
              if (!err) {
                ...
                _data.read('users', checkData.userPhone, function(err, userData) {
                  if (!err && userData) {
                    ...
                    callback(500, {'Error' : 'Could not fine the user  who created the check, so could not remove the check from the list of checks on the user object!'});
                  }
                });
              } else {
